...
ADMIN_ID = 7436995277  # Replace with the admin's Telegram ID
...
async def start(event):
    ...
    if not await is_user_in_channel(user_id):
        await event.reply(
            ...
            buttons=[Button.url("𝗣𝗘𝗥𝗠 𝗣𝗢𝗥𝗧𝗔𝗟", f"https://t.me/pvtshits")]
        )
        return
    ...
    await event.reply("✅️Omfo Niggah; You've Joined Our Channel So Now You Can Send /report Here To Start Reporting Your Target😈.")
...
async def report_command(event):
    ...
    if not await is_user_in_channel(user_id):
        await event.reply(
            f"😶‍🌫️Heya **[{full_name}]**({permanent_link}) Must Joine Our Premium Channel To Use This Bot",
            buttons=[Button.url("𝗣𝗘𝗥𝗠 𝗣𝗢𝗥𝗧𝗔𝗟", f"https://t.me/{CHANNEL_USERNAME}")]
        )
        return
